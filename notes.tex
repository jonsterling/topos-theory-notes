...
more intuitive internal \emph{monotonicity} condition that $j$ shall
...
\begin{proof}
  ...
  ($\Leftarrow$) Suppose that $j$ preserves implication; we need to
...
\end{thm}
\begin{proof}
  We will prove the above facts in reverse order. First, to show that
  $\AreAdjoint{\Sheafify,\iota}$, let us begin by explicitly
  constructing candidate units and the counits for the adjunction.
  \begin{gather*}
    \begin{aligned}
      \AOf{
        \eta
    ...
    \begin{aligned}
      \AOf{
        ...
        \Sheafify\iota\to\ArrId{\Sh{\mathbb{C},J}}
      }
      \\
      \ADefine{
        \epsilon_{\mathcal{F}}^\Psi(m)
      }{
        \Glue{\mathcal{F}}^\Psi(\Glue{\Plus{\mathcal{F}}}^\Psi(m))
      }
    \end{aligned}
  \end{gather*}
  Now we have to show that the following identities hold:
  \[
    \begin{tikzcd}[sep=large]
      \IMode{\Sheafify}
      \arrow[r,"\IMode{\Sheafify\eta}"]
      \arrow[rr,swap,bend right=40,"\IMode{\ArrId{\Sheafify}}"]
      &
      \IMode{\Sheafify\iota\Sheafify}
      \arrow[r,"\IMode{\epsilon\Sheafify}"]
      &
      \IMode{\Sheafify}
    \end{tikzcd}
    \qquad
    \begin{tikzcd}[sep=large]
      \IMode{\iota}
      \arrow[r,"\IMode{\eta\iota}"]
      \arrow[rr,swap,bend right=40,"\IMode{\ArrId{\iota}}"]
      &
      \IMode{\iota\Sheafify\iota}
      \arrow[r,"\IMode{\iota\epsilon}"]
      &
      \IMode{\iota}
    \end{tikzcd}
  \]
