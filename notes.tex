...
\begin{lem}[Equality in $\Plus{P}$]
  ...
  \item There exists some common refinement $\IsSubsetEq{T}{R\cap S}$ such that for all $\Member{\IBox{\Of{\psi}{\Phi\to\Psi}}}{T(\Phi)}$, we have $\IsEqOf{\mathfrak{m}(\psi)}{\mathfrak{n}(\psi)}{P(\Phi)}$.
  \end{enumerate}
\end{lem}
\begin{proof}
  (1)~$\Leftrightarrow$~(2) is the definition of equality for a colimit in
  $\Sets$. % To show (1) $\Rightarrow$ (3), observe that we have both $\IsEqOf{(R\cap S)}{?}{?}$
