...
including \IMode{Platonism} (the \OMode{\emph{category of sets}}),
\IMode{constructivism} (the \OMode{\emph{free topos}}),
\IMode{intuitionism} (the \OMode{\emph{topos of sheaves over the
    universal spread}})\footnote{There are other options too, such as
  the gros topos of sheaves over the category of separable locales
  equipped with the open cover
  topology~\cite{fourman:1984,fourman:2013}.} and \IMode{vulgar
  constructivism} (the \OMode{\emph{effective topos}}).
...
%  Therefore, we
%   can form a new cone
%   $\MkCone{f^*A}{\alpha_i^\sharp:f^*A\to D_i}{i:\mathbb{J}}$ in
%   $\mathbb{F}$. Now we need to show that this cone is a
%   \emph{limiting} cone.

%   Fix another cone $\MkCone{B}{\beta_i:B\to D_i}{i:\mathbb{J}}$ in
%   $\mathbb{F}$; we need to exhibit a unique morphism
%   $\Of{u}{B\to f^*A}$ such that for all $\Of{i}{\mathbb{J}}$, we have
%   $\IsEqOf{\beta_i}{\alpha_i^\sharp\circ u}{B\to D_i}$. Observe that
%   we can turn $B$ into a cone in $\mathbb{E}$, namely
%   $\MkCone{f_*B}{f_*\beta_i:f_*B\to f_*D_i}{i:\mathbb{J}}$, and by the
%   universal property of the limit in $\mathbb{E}$ we have a unique
%   $\Of{v}{f_*B\to A}$ such that
%   $\IsEqOf{f_*\beta_i}{\alpha_i\circ v}{f_*B\to f_*D_i}$.

%   Lifting $v$ through the reflector, we have
%   $\Of{f^*v}{\square B\to f^*A}$ in $\mathbb{F}$. Because the
%   coreflector $\Of{f_*}{\mathbb{F}\hookrightarrow\mathbb{E}}$ is full
%   and faithful, the counit $\Of{\epsilon_B}{\square B\to B}$ is an
%   isomorphism. Therefore, we can define our desired mediating map in
%   $\mathbb{F}$ as $\Define{u}{f^*v\circ\epsilon_B^{-1}}$.
% %
%   Because the coreflector is faithful, to show that
%   $\IsEq{\beta_i}{\alpha_i^\sharp\circ u}$ in $\mathbb{F}$, it
%   suffices to show that
%   $\IsEq{f_*\beta_i}{f_*(\alpha_i^\sharp\circ u)}$ in $\mathbb{E}$:
%   \begin{align*}
%     \AIsEq{
%       f_*\beta_i
%     }{
%       f_*\alpha_i^\sharp\circ f_*u
%     }
%     \\
%     \AIsEq{
%     }{
%       f_*(\epsilon_{D_i}\circ f^*\alpha_i) \circ f_*u
%     }
%     \tag{definition of transpose}
%     \\
%     \AIsEq{
%     }{
%       f_*(\epsilon_{D_i})\circ \bigcirc \alpha_i \circ f_*u
%     }
%     \tag{distribute coreflector}
%     \\
%     \AIsEq{
%     }{
%       f_*(\epsilon_{D_i})\circ \bigcirc \alpha_i \circ f_*(f^*v\circ\epsilon_B^{-1})
%     }
%     \tag{definition of $u$}
%     \\
%     \AIsEq{
%     }{
%       f_*(\epsilon_{D_i})\circ \bigcirc (\alpha_i \circ v)\circ f_*(\epsilon_B^{-1})
%     }
%     \tag{reassociate}
%     \\
%     \AIsEq{
%     }{
%       f_*(\epsilon_{D_i})\circ \bigcirc (\alpha_i \circ v)\circ \eta_{f_*B}
%     }
%     \tag{triangle identity}
%     \\
%     \AIsEq{
%     }{
%       f_*(\epsilon_{D_i})\circ \eta_{f_*D_i}\circ \alpha_i \circ v
%     }
%     \tag{naturality of $\eta$}
%     \\
%     \AIsEq{
%     }{
%       \alpha_i\circ v
...
%   The fact that $u$ is unique follows from the uniqueness of
%   $v$. \textbf{(TODO, is that true?)}
\end{proof}

% \paragraph{Sheaf subtoposes}
% So far we have shown in the previous section that $\Sh{\mathbb{C},J}$
% is a lex reflective subcategory of $\Psh{\mathbb{C}}$; we will now see
% that this implies that it is also a topos.

% \begin{thm}
%   A lex reflective subcategory of a topos is again a topos.
% \end{thm}
% \begin{proof}
%   Fix a topos $\Match{\mathfrak{E}}{\Psh{\mathbb{C}}}$ along with a
...
%     \end{tikzcd}
%   \]
%   We need to show that $\mathfrak{F}$ is a topos, which is to say that
%   $\mathfrak{F}$ has finite limits, is cartesian closed, and has a
%   subobject classifier. To see that $\mathfrak{F}$ has finite limits,
%   observe that the reflector $\Of{f^*}{\mathfrak{E}\to\mathfrak{F}}$
%   preserves
% \end{proof}
